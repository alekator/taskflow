...
function bounds(el: TaskRoadmapElement) {
  ...
  if (el.type === "arrow") {
    ...
    return {
      x: Math.min(el.x, tx),
      y: Math.min(el.y, ty),
      w: Math.abs(tx - el.x) || 1,
      h: Math.abs(ty - el.y) || 1,
    };
  }
  ...
  return {
    x: el.x,
    y: el.y - 18,
    w: Math.max(40, (el.text?.length ?? 4) * 8),
    h: 24,
  };
}
...
export function TaskRoadmapPanel({ taskId }: { taskId: string }) {
  ...
  const world = useCallback((cx: number, cy: number) => {
    ...
    return {
      x:
        (cx - r.left - docRef.current.viewport.x) /
        docRef.current.viewport.zoom,
      y:
        (cy - r.top - docRef.current.viewport.y) / docRef.current.viewport.zoom,
    };
  }, []);

  const saveSoon = useCallback(
    (next: TaskRoadmapData) => {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const arrowHead = (
        ...
        ctx.moveTo(x2, y2);
        ctx.lineTo(
          x2 - sz * Math.cos(a - Math.PI / 6),
          y2 - sz * Math.sin(a - Math.PI / 6),
        );
        ctx.lineTo(
          x2 - sz * Math.cos(a + Math.PI / 6),
          y2 - sz * Math.sin(a + Math.PI / 6),
      ...
      doc.elements.forEach((el) => {
        ...
        if (el.type === "path" && el.points && el.points.length > 1) {
          ...
          ctx.strokeRect(
            sx(el.x),
            sy(el.y),
            (el.width ?? 1) * doc.viewport.zoom,
            (el.height ?? 1) * doc.viewport.zoom,
          );
        } else if (el.type === "arrow") {
          const tx = el.toX ?? el.x;
          const ty = el.toY ?? el.y;
          ctx.beginPath();
          ctx.moveTo(sx(el.x), sy(el.y));
          ...
          if (cached && cached.complete) {
            ctx.drawImage(
              ...
              sx(el.x),
              sy(el.y),
              (el.width ?? 320) * doc.viewport.zoom,
              (el.height ?? 180) * doc.viewport.zoom,
            );
          } else {
            const im = new Image();
            im.src = el.imageDataUrl;
            im.onload = () => {
              imgsRef.current.set(el.imageDataUrl as string, im);
              draw();
            };
        ...
        if (selected === el.id) {
          ...
          ctx.strokeRect(
            sx(b.x),
            sy(b.y),
            b.w * doc.viewport.zoom,
            b.h * doc.viewport.zoom,
          );
          ctx.setLineDash([]);
          if (el.type === "rect" || el.type === "image") {
            const hx = sx(b.x + b.w);
  ...
  useEffect(() => {
    ...
    const all = doc.elements.length
      ? doc.elements.map(bounds)
      : [{ x: -300, y: -180, w: 600, h: 360 }];
  ...
  const onMove: React.PointerEventHandler<HTMLCanvasElement> = (e) => {
    ...
    if (st.mode === "pan") {
      ...
      setDocSafe((prev) => ({
        ...
        viewport: {
          ...prev.viewport,
          x: st.ox + (e.clientX - st.sx),
          y: st.oy + (e.clientY - st.sy),
        },
      }));
      return;
    }
    ...
    if (st.mode === "rect") {
      setDocSafe((prev) => ({
        ...prev,
        elements: prev.elements.map((el) =>
          el.id === st.id && el.type === "rect"
            ? {
                ...el,
                x: Math.min(st.x, p.x),
                y: Math.min(st.y, p.y),
                width: Math.abs(p.x - st.x),
                height: Math.abs(p.y - st.y),
              }
            : el,
        ),
      }));
    ...
    if (st.mode === "resize") {
      setDocSafe((prev) => ({
        ...prev,
        elements: prev.elements.map((el) =>
          el.id === st.id && (el.type === "rect" || el.type === "image")
            ? {
                ...el,
                x: st.x,
                y: st.y,
                width: Math.max(20, p.x - st.x),
                height: Math.max(20, p.y - st.y),
              }
            : el,
        ),
      }));
  ...
  const commitText = () => {
    ...
    if (!v) {
      patchDoc((prev) => ({
        ...prev,
        elements: prev.elements.filter((el) => el.id !== textEditor.id),
      }));
      setSelected(null);
      setTextEditor(null);
      return;
    ...
    patchDoc((prev) => ({
      ...prev,
      elements: prev.elements.map((el) =>
        el.id === textEditor.id && el.type === "text" ? { ...el, text: v } : el,
      ),
    }));
    setTextEditor(null);
  };
  ...
  const deleteSelected = () => {
    ...
    patchDoc((prev) => ({
      ...prev,
      elements: prev.elements.filter((el) => el.id !== selected),
    }));
    setSelected(null);
  };
    ...
    <section className="item-card task-roadmap-panel">
      ...
      >
        <div>
          ...
          <p className="soft">
            {open
              ? "Shared board for diagrams and notes."
              : "Click to open infinite canvas for this task"}
          </p>
        </div>
        ...
        <div className="task-roadmap-workspace">
          <div className="task-roadmap-toolbar">
            <div className="task-roadmap-tools">
              {(
                ["select", "pan", "pen", "rect", "arrow", "text"] as Tool[]
              ).map((t) => (
                <button
                  key={t}
                  type="button"
                  className={`button button-ghost button-compact${tool === t ? " task-roadmap-tool-active" : ""}`}
                  onClick={() => {
                    setTool(t);
                    setTextEditor(null);
                  }}
            ...
            <div className="task-roadmap-toolbar-settings">
              <label className="task-roadmap-color-label">
                Color
                <input
                  type="color"
                  value={stroke}
              ...
              <label className="task-roadmap-width-label">
                Width
                <input
                  type="range"
                  min={1}
          ...
          {loading ? (
            ...
            <div ref={shellRef} className="task-roadmap-canvas-shell">
              ...
              <aside className="task-roadmap-minimap">
                <div className="task-roadmap-minimap-head">
                  <span className="meta">Minimap</span>
                  <span className="meta">
              ...
              {textEditor ? (
                <div
                  className="task-roadmap-text-editor"
                  style={{
          ...
          <div className="task-roadmap-footnote">
            <span className="meta">
              Pro mode: select/move objects, resize shape/image corners, inline
              text edit and minimap viewport.
            </span>
          </div>
        </div>
      ) : null}
